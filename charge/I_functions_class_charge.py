...
def _cf(x,x0,x1):
    if x<x0: return 0.
    if x>x1: return 1.
    return np.sin(np.pi/2.*(x-x0)/(x1-x0))**2
...
class ElectronWaveFunction:
    ...
    def __init__(self, nu, h, k, mu, M, lam, GC, c, tol, aZ=0., trunc={}):
    ...
    def diff_eq(self, r_star, F, G):
        ...
        potential_energy = -self.aZ/r
        if 'r' in self.trunc:
            potential_energy = potential_energy*_cf(r_star,self.trunc['r'][0],self.trunc['r'][1])\
                                   *_cf(-r_star,-self.trunc['r'][3],-self.trunc['r'][2])

        dG = ((self.h - potential_energy - self.mu * np.sqrt(1 - (2*self.M / r)))* F - 
            (self.k / r) * np.sqrt(1 - (2*self.M / r)) * G) 
        
        dF = ((-self.mu * np.sqrt(1 - (2*self.M / r)) - self.h + potential_energy) * G + 
