...
class IfunctionsNoM:
    ...
    def __init__(self,k,h,h_prime,M):
        self.k=k
        self.h=h
        ...
        self.coeff = 1./np.sqrt(4*h*h_prime)


    def Ione(self,r,r_points,F_points_xkh, G_points_xkh,F_points_xkh_prime,G_points_xkh_prime):
        ...
        g_xprime_k_hprime = np.array(G_points_xkh_prime)
        
        fstar_xkh = np.conjugate(np.array(F_points_xkh))
        f_xprime_k_hprime = np.array(F_points_xkh_prime)
            

        rdependentpart = 2*self.M/r
        
        line = (fstar_xkh*f_xprime_k_hprime + gstar_xkh*g_xprime_k_hprime)*rdependentpart
           
        rstar_step = r_points[0]-r_points[1] #switched order to get positive steps in
        

            
        intofline = (np.sum(line[1:-1])+.5*(line[0]+line[-1]))*rstar_step


        return (intofline*self.coeff)  
    

    def Itwo(self,r,r_points,F_points_xkh, G_points_xkh,F_points_xminkh_prime,G_points_xminkh_prime):
        #need analytic continuation of wavefunctions! 
        #can just add the analytic piece at the end! so need to do that also
            
        gstar_xkh = np.conjugate(np.array(G_points_xkh))
        gstar_xprime_mink_hprime = np.conjugate(np.array(G_points_xminkh_prime))
        
        fstar_xkh = np.conjugate(np.array(F_points_xkh))
        fstar_xprime_mink_hprime = np.conjugate(np.array(F_points_xminkh_prime))
            

        rdependentpart = 2*self.M/r
        
        line = (fstar_xkh*gstar_xprime_mink_hprime + gstar_xkh* fstar_xprime_mink_hprime)*rdependentpart
