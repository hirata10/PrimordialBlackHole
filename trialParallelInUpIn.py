...
#.read(rows=[1,5], columns=['index','x','y'])
#print(hduP)
r_points_gamma = hduP[omega_index]['rpoints_up'][:]
 
psi_gammalomega = hduP[omega_index]['F_points_in'][:]
psi_gammalomega_prime = hduP[omega_index]['z_points_in'][:]
...
def prof_make_tables():
    ...
    for k in range(len(ks)):
        ...
        F_points_xminkh = hdu[h_index].data.field('F_points_in')
        G_points_xminkh = hdu[h_index].data.field('G_points_in')
        #print("in")
